...
<div class="chart {size}" bind:this={el}>
  <div class="background">
    <Pancake.Chart x1={$x1 - 2.5} x2={$x2 + 2.5} y1={0} y2={max} clip>
      ...
      <Pancake.Grid vertical ticks={birth_years} let:value>
        <span
          class="x label">{size === 'large' ? value : `'${String(value).slice(2)}`}</span>
      </Pancake.Grid>
    </Pancake.Chart>
  </div>
    ...
    <Pancake.Chart x1={90 + 2.5} x2={0 - 2.5} y1={0} y2={max}>
      ...
      <Pancake.Grid vertical count={size === 'large' ? 20 : 10} let:value>
        <span class="x label">
          {value}
          {#if value === 0}
            <span style="position: absolute; left: 2.5em">yrs old</span>
          {/if}
        </span>
      </Pancake.Grid>
    </Pancake.Chart>
  ...
  <div class="slider-container">
    ...
    <button
      disabled={year === year0}
      on:click={() => (year -= 10)}>&larr;</button>
    <span on:pointerdown={handle_pointerdown}>{year}</span>
    <button
      disabled={year === year1}
      on:click={() => (year += 10)}>&rarr;</button>
  </div>
</div>

<p class="credit">
  Source:
  <a href="https://ipums.org/">IPUMS</a>. Based on
  <a href="https://bl.ocks.org/mbostock/4062085">Population Pyramid by Mike
    Bostock</a>
</p>
